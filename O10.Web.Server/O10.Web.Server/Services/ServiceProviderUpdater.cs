...
using Flurl.Http;
using O10.Web.Server.Dtos.ServiceProvider;
...
namespace O10.Web.Server.Services
{
    public class ServiceProviderUpdater : IUpdater
    {
        ...
        private async Task VerifyProofToAssociatedAttributeKnowledge(UniversalProofs universalProofs, string schemeName)
        {
            ...
            bool res = await _gatewayService.AreAssociatedAttributesExist(universalProofs.Issuer.Value, attrs).ConfigureAwait(false);
            if (!res)
            {
        ...
        private async Task ProcessUniversalProofsAuthentication(UniversalProofs universalProofs)
        {
            IEnumerable<SpIdenitityValidation> spIdenitityValidations = _dataAccessService.GetSpIdenitityValidations(_accountId);
            IKey commitment = universalProofs.IssuersAttributes.FirstOrDefault(a => a.Issuer.Equals(universalProofs.Issuer))?.RootAttribute.Commitment;
            try
            {
                await _spValidationsService.CheckAssociatedProofs(commitment.Value, universalProofs.IssuersAttributes, spIdenitityValidations).ConfigureAwait(false);
            }
            ...
            if (isNew)
            {
                ...
                await _idenitiesHubContext
                    .Clients.Group(universalProofs.SessionKey)
