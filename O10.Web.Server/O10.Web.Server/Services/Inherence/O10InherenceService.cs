...
namespace O10.Web.Server.Services.Inherence
{
    ...
    public class O10InherenceService : IInherenceService
    {
        ...
        private async Task ProcessUniversalTransport(UniversalTransport universalTransport)
        {
            ...
            try
            {
                ...
                SetCompletion(
                    new InherenceData
                    {
                        Issuer = universalProofs.Issuer.ArraySegment.Array,
                        AssetRootCommitment = commitmentKey.ArraySegment.Array,
                        RootRegistrationProof = registrationProof,
                        AssociatedRootCommitment = universalProofs.IssuersAttributes?.FirstOrDefault(a => !a.Issuer.Equals(universalProofs.Issuer))?.RootAttribute.Commitment.ArraySegment.Array,
                        AssociatedRegistrationProof = universalProofs.IssuersAttributes?.FirstOrDefault(a => !a.Issuer.Equals(universalProofs.Issuer))?.RootAttribute.CommitmentProof.SurjectionProof
        ...
        private async void ProcessIdentityProofs(IdentityProofs packet)
        {
            ...
            (long registrationId, bool isNew) = _spValidationsService.HandleAccount(AccountId, packet.AssetCommitment, packet.AuthenticationProof);
